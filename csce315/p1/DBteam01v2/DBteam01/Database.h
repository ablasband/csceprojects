...
public:
	...
	*/
	DBTEAM01_API bool addTable(std::string name, Table table);
	...
	*/
	DBTEAM01_API bool dropTable(std::string name);
	...
	*/
	...
	DBTEAM01_API Table query (std::vector<std::string> const&,
	...
	/// @throw InvalidType The where expression tries to compare incompatible types
	/// @throw InvalidQuery The where expression cannot be parsed (invalid syntax).
	DBTEAM01_API void deleteByQuery (std::vector<std::string> const& select,
		std::string const& from, std::string const& where);
