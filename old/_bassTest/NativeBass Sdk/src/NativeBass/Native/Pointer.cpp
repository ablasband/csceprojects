...
jclass getPointerClass(JNIEnv *jenv) {
	if(!pointerClass) {
		pointerClass = (jclass)jenv->NewGlobalRef(jenv->FindClass("org/jouvieje/bass/misc/Pointer"));
	}
	return pointerClass;
}
...
jmethodID getNewPointerID(JNIEnv *jenv) {
	if(!newPointerID) {
		newPointerID = jenv->GetStaticMethodID(getPointerClass(jenv), "newPointer", "(J)Lorg/jouvieje/bass/misc/Pointer;");
	}
	return newPointerID;
}
