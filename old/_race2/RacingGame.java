...
public class RacingGame extends Applet implements Runnable {
	...
	public RacingGame() {
		...
		Color backCol = new Color(192,192,192);
	...
	public void paintTrackImage( Graphics g ) {
		...
		double ang = vehicle.currentState().angle + vehicle.steering_angle;
		...
		if (showfps)															//DEBUG DRAWING
		{		
			for (int i = 1; i < 6; i ++)
			{
				g.setColor(Color.black);
				
				if (vehicle.rpms > 1000*i)
				{				
					g.setColor(Color.green);
					
					if (vehicle.rpms >= 4000 && i == 4)
					{
						g.setColor(Color.orange);
					}
					
					if (vehicle.rpms >= 5000 && i == 5)
					{
						g.setColor(Color.red);
					}				
				}
				
				g.drawString( Character.toString(dot),textX + 14*(i-1), textY - 40 );
			}
		
			g.setColor(Color.black);
		
			//g.drawString( "angle " + Double.toString(ang), 
			//			 textX, textY - 80 );
			
			g.drawString( "backwards " + Boolean.toString(vehicle.backwards), 
						 textX, textY - 80 );
						 
			//g.drawString( "force " + Float.toString(vehicle.forceDisp), 
			//			 textX, textY - 60 );
			
			g.drawString( Float.toString(vehicle.currentState().vx) + "," + Float.toString(vehicle.currentState().vy), 
						 textX, textY - 60 );
						 
			
			g.drawString( "rpms " + Float.toString(vehicle.rpms), 
						 textX, textY - 100 );
						 
			g.drawString( "motoron " + Boolean.toString(vehicle.motoron), 
						 textX, textY - 120 );
			...
			if (vehicle.gear > 0)
			{
				gearStr = Integer.toString(vehicle.gear);
			}
			else
			{
				gearStr = "R";
		...
		else																	//NORMAL DRAWING
		{
			for (int i = 1; i < 6; i ++)
			{
				...
				g.drawString( Character.toString(dot),textX + 14*(i-1) - 220, textY + 40 );
			}
			...
			g.drawString( gearStr, 
						 textX - 145, textY + 38 );
						 
			g.setColor(Color.black);
			
			Float speed = vehicle.currentState().speed() * 1.90f;				//gives a rough mph
						 
			g.drawString( Float.toString(speed).substring(0,2) + " mph", 
						 textX + 0, textY + 38 );
		...
		if ( race.getCurrentLight() != 4 ) {
			...
			switch( light ) {
				case 1: numGreen = 1; break;
				case 2: numGreen = 2; break;
				case 3: numGreen = 3; break;
				default:
					numGreen = 1;
			}
			
			g.setColor(Color.green);
			g.fillOval( clipPoint.x + size.width/2 - 10 - 60, clipPoint.y + size.height/2 - 10, 30, 30 );
			
			g.setColor(Color.red);
			
			if (numGreen == 2 || numGreen == 3)
			{
				g.setColor(Color.green);
			}
			
			g.fillOval( clipPoint.x + size.width/2 - 10, clipPoint.y + size.height/2 - 10, 30, 30 );
	...
	public void paintTrack( Graphics g ) {
		...
		for ( int j = 0; j < vehicles.length; j++ ) {
			...
			int len = 4;
			g.drawLine( (int)(state.x + vehicle.wx1), (int)(state.y + vehicle.wy1),			//wheels
	...
	public static void main( String[] args ) {
		...
		Button host = new Button( "Play" );										//modified from "Host"
